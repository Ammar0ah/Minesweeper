...
public class NormalGame extends Game {
    @Override
    public boolean acceptMove(playerMove playerMove, moveType moveType, ArrayList<Player> players) {

        try {
            Square cell = playerMove.getSquare();
            int i = cell.getI();
            int j = cell.getJ();
            if (cell.getState() == squareState.STATE_CLOSED && (moveType == moveType.Mark)) {
                ...
                if(players.size() == 1)
                    cell.setMark("$");
                return false;
            } else if (cell.getState() == squareState.STATE_CLOSED && (moveType == moveType.Reveal)) {
                openBlankCells(i, j,playerMove);
                return true;
            } else if (cell.getState() == squareState.STATE_FLAG && (moveType == moveType.Unmark)) {
                cell.setMark("#");
                ...
                return acceptMove(p,moveType,players);
            }
